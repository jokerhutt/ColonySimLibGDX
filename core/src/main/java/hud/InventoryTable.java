...
public class InventoryTable extends Table {
    ...
    public InventoryTable(GameScreen screen) {
        this.squareTexture = new Texture("brown.png");
...
//        this.add(textBox).size(320, 100).colspan(3).center(); // <-- the key fix
        ...
        for (int i = 0; i < 12; i++) {
            GameItem item = (i < screen.player.inventory.inventoryArray.length) ? screen.player.inventory.inventoryArray[i] : null;
